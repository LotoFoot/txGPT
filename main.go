...
import (
	...
	"github.com/LotoFoot/txGPT/features" // Import complet pour features/ (basé sur votre repo GitHub)
)
...
func main() {
	...
	if *agent {
		client := openai.NewClient(os.Getenv("OPENAI_API_KEY"))
		response := features.AgentMode(client, prompt, 5) // 5 itérations max
...
// Vos fonctions existantes (interactiveMode, interactiveShellMode, etc.) inchangées...
// Collez-les ici si nécessaire, mais elles n'ont pas changé.
